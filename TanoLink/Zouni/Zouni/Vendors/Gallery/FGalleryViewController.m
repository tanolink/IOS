...
- (void)loadView
{
    ...
    _captionOfCurrPageNum               = [[UILabel alloc] init];
    _captionOfTotalCount                = [[UILabel alloc] init];
    //_labPraise                          = [[UILabel alloc] init];
    ...
    _captionOfCurrPageNum.font                  = DEFAULT_FONT(16);
    _captionOfCurrPageNum.textColor				= [UIColor whiteColor];
    _captionOfCurrPageNum.backgroundColor		= [UIColor clearColor];
    _captionOfCurrPageNum.textAlignment			= NSTextAlignmentRight;
    _captionOfCurrPageNum.shadowColor			= [UIColor blackColor];
    _captionOfCurrPageNum.shadowOffset			= CGSizeMake( 1, 1 );
    
    _captionOfTotalCount.font                 = DEFAULT_FONT(12);
    _captionOfTotalCount.textColor			= [UIColor whiteColor];
    _captionOfTotalCount.backgroundColor		= [UIColor clearColor];
    _captionOfTotalCount.textAlignment			= NSTextAlignmentLeft;
    _captionOfTotalCount.shadowColor			= [UIColor blackColor];
    _captionOfTotalCount.shadowOffset			= CGSizeMake( 1, 1 );
    ...
    [_captionContainer addSubview:_captionOfCurrPageNum];
    [_captionContainer addSubview:_captionOfTotalCount];
    /*
...
- (void)updateCaption
{
	if([_photoSource numberOfPhotosForPhotoGallery:self] > 0 )
	{
		if([_photoSource respondsToSelector:@selector(photoGallery:captionForPhotoAtIndex:)])
		{
			...
			if([caption length] > 0 )
			{
                float captionPageControlWidth = 50;
                float captionWidth = _container.frame.size.width-kCaptionPadding*3 - captionPageControlWidth;
                ...
                float pageNumberHeight = 20;
				_captionOfCurrPageNum.frame = CGRectMake(CGRectGetMaxX(_caption.frame)+kCaptionPadding,
                                                         _caption.frame.origin.y+height/2-pageNumberHeight/2,
                                                         25,
                                                         pageNumberHeight);
                _captionOfTotalCount.frame = CGRectMake(CGRectGetMaxX(_captionOfCurrPageNum.frame),
                                                        _captionOfCurrPageNum.frame.origin.y+kCaptionPadding,
                                                        25,
                                                        pageNumberHeight - kCaptionPadding);
...
- (void)updatePageControl{
    [_captionOfTotalCount setText:[NSString stringWithFormat:@"%@%i",NSLocalizedString(@"/",@""),[_photoSource numberOfPhotosForPhotoGallery:self]]];
    [_captionOfCurrPageNum setText:[NSString stringWithFormat:@"%d",_currentIndex+1]];
}
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
	if([self.selectedViewController isKindOfClass:[UINavigationController class]])
	{
		...
		if([navController.visibleViewController isKindOfClass:[FGalleryViewController class]])
		{
			...
			[galleryController resetImageViewZoomLevels];
            [galleryController layoutButtons];
		}
	}
}
