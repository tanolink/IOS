...
-(void) setDefaultSelectedWithValue:(NSString *)defaultString{
    if (_isLevelDic){
        if(defaultString.length>0){
            _state = [defaultString componentsSeparatedByString:@" "][0];
            int row = 0;
            for (int i = 0; i<_dicKeyArray.count; i++) {
                NSString *stateStr = _dicKeyArray[i][0];
                if ([stateStr isEqualToString:(NSString *)_state]) {
                    row = i;
                    break;
                }
            }
            ...
            if (dicValueArray.count>0) {
                _city = [defaultString componentsSeparatedByString:@" "][1];
                NSUInteger i = [dicValueArray indexOfObject:_city];
                @try {
